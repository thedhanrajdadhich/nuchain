...
For a version without any/all of these restrictions, please contact us at [dhanraj@nuchain.co](mailto:dhanraj@nuchain.co).
...
Servers:
node0: 127.0.0.1:8000 ["Water", sending: True]
node1: 127.0.0.1:8001 ["Air", sending: True]
node2: 127.0.0.1:8002 ["Fire", sending: True]
node3: 127.0.0.1:8003 ["Earth", sending: True]
node0> server node1
node1>
```
...
Refer to "Entity Configuration" below for general notes on privacy configurations. This demo requires that there be 4 entities configured by the `genconfs` tool, which will name them "Water", "Air", "Fire" and "Earth". These would correspond to business entities on the blockchain, communicating with private messages over the blockchain. Confirm this setup with the `server` command.
...
Create the private accounts by sending a _private message_ that executes a multi-step _pact_ to create private accounts on each entity. Change to Water's server (node0) and send a private message to the other 3 participants with the demo code:

```
node3> server node0
node0> private Water [Air Fire Earth] (demo.create-private-accounts)
```

The `private` command creates an encrypted message, sent from Water to Air, Fire and Earth. The `create-private-accounts` pact executes a single command on the different servers. To see the results, perform _local queries_ on each node.
...
Now, execute a confidential transfer between Water and Air, transferring money from Water's account "A" to Air's account "B".
...
Set the server to node0 for Water, and execute the pact to send 1.00 to Air:

```
node3> server node0
node0> private Water [Air] (demo.payment "Water" "A" "Air" "B" 1.00)
status: success
data:
  amount: '1.00'
...
You can also test out the rollback functionality on an error. Mistype the recipient account id (in this case we use "bad" instead of "B"). The pact will execute the debit on Water/node0; attempt the credit on Air/node1, failing because of the bad ID; finally the rollback will execute on Water/node0. Air's account will be unchanged, while Water's account will note the rollback with the original tx id of the pact execution.

```
node0> private Water [Air] (demo.payment "Water" "A" "Air" "bad" 1.00)
status: success
data:
  tx: 7
  amount: '1.00'
