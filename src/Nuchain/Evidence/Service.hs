...
module Nuchain.Evidence.Service
  ( runEvidenceService
  , initEvidenceEnv
...
import Nuchain.ConfigChange as CC
import Nuchain.Event (pprintBeat)
import Nuchain.Evidence.Spec as X
...
import Nuchain.Types.Evidence (Evidence(..), PublishedEvidenceState(..), CommitCheckResult (..))
import Nuchain.Types.Message (AppendEntriesResponse(..))
import Nuchain.Util.Util (linkAsyncTrack, throwDiagnostics)
...
runEvidenceProcessor = do
  ...
  case newEv of
    VerifiedAER aers -> do
      ...
      put newEs
      publishEvidence
      when (newEs ^. esResetLeaderNoFollowers) tellNuchainToResetLeaderNoFollowersTimeout
...
tellNuchainToResetLeaderNoFollowersTimeout :: EvidenceService EvidenceState ()
tellNuchainToResetLeaderNoFollowersTimeout = do
