...
module Nuchain.Consensus.Handle.HeartbeatTimeout
    (handle)
where
...
import Nuchain.Consensus.Handle.AppendEntries (clearLazyVoteAndInformCandidates)
import qualified Nuchain.Types.Sender as Sender (ServiceRequest(..), AEBroadcastControl(..))
